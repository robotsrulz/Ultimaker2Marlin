  ...
  while( MYSERIAL.available() > 0  && buflen < BUFSIZE) {
       ...
       serial_count >= (MAX_CMD_SIZE - 1) )
    {
      ...
      if(!comment_mode){
        ...
        if (strchr_pointer && strtol(&cmdbuffer[bufindw][strchr_pointer - cmdbuffer[bufindw] + 1], NULL, 10) != 105)
...
void process_commands()
{
  ...
  else if(code_seen('M'))
  {
    ...
    SERIAL_PROTOCOLLNPGM(MSG_M119_REPORT);
        ...
        SERIAL_PROTOCOLLN(((READ(X_MIN_PIN)^X_ENDSTOPS_INVERTING)?PSTR(MSG_ENDSTOP_HIT):PSTR(MSG_ENDSTOP_OPEN)));
      #endif
      #if defined(X_MAX_PIN) && X_MAX_PIN > -1
        SERIAL_PROTOCOLPGM(MSG_X_MAX);
        SERIAL_PROTOCOLLN(((READ(X_MAX_PIN)^X_ENDSTOPS_INVERTING)?PSTR(MSG_ENDSTOP_HIT):PSTR(MSG_ENDSTOP_OPEN)));
      #endif
      #if defined(Y_MIN_PIN) && Y_MIN_PIN > -1
        SERIAL_PROTOCOLPGM(MSG_Y_MIN);
        SERIAL_PROTOCOLLN(((READ(Y_MIN_PIN)^Y_ENDSTOPS_INVERTING)?PSTR(MSG_ENDSTOP_HIT):PSTR(MSG_ENDSTOP_OPEN)));
      #endif
      #if defined(Y_MAX_PIN) && Y_MAX_PIN > -1
        SERIAL_PROTOCOLPGM(MSG_Y_MAX);
        SERIAL_PROTOCOLLN(((READ(Y_MAX_PIN)^Y_ENDSTOPS_INVERTING)?PSTR(MSG_ENDSTOP_HIT):PSTR(MSG_ENDSTOP_OPEN)));
      #endif
      #if defined(Z_MIN_PIN) && Z_MIN_PIN > -1
        SERIAL_PROTOCOLPGM(MSG_Z_MIN);
        SERIAL_PROTOCOLLN(((READ(Z_MIN_PIN)^Z_ENDSTOPS_INVERTING)?PSTR(MSG_ENDSTOP_HIT):PSTR(MSG_ENDSTOP_OPEN)));
      #endif
      #if defined(Z_MAX_PIN) && Z_MAX_PIN > -1
        SERIAL_PROTOCOLPGM(MSG_Z_MAX);
        SERIAL_PROTOCOLLN(((READ(Z_MAX_PIN)^Z_ENDSTOPS_INVERTING)?PSTR(MSG_ENDSTOP_HIT):PSTR(MSG_ENDSTOP_OPEN)));
      #endif
      break;
